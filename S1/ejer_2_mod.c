...
int main(int argc, char *argv[]){
    ...
    //Devolvemos el puntero de escritura al principio
    if((lseek(fd_output, 0, SEEK_SET)) < 0){
        printf("\n[-] ERROR: colocación del puntero de escritura", errno);
        perror("\n[-] ERROR EN LSEEK");
        exit(-1);
    }
    
    //Escribimos en el fichero
    dprintf(fd_output, "Bloques totales: %d\n", num_bloque);
